...
SIFTALPHA_PYTHON_REQUIRES_CHECK
            ...
            # environment for rollback, but build the replacement directly at its final path.
            backup="${'$'}venv.backup-${'$'}${'$'}"
            ready_backup="${'$'}ready.backup-${'$'}${'$'}"
            rm -rf -- "${'$'}backup"
            rm -f -- "${'$'}ready_backup"
            had_previous_env=0
            had_previous_ready=0

            if [ -e "${'$'}ready" ]; then
              if ! mv -- "${'$'}ready" "${'$'}ready_backup"; then
                echo 'SIFTALPHA_ERROR=ENVIRONMENT_ACTIVATION_FAILED'
                exit 76
              fi
              had_previous_ready=1
            fi

            if [ -e "${'$'}venv" ]; then
              if ! mv -- "${'$'}venv" "${'$'}backup"; then
                if [ "${'$'}had_previous_ready" -eq 1 ] && [ -e "${'$'}ready_backup" ]; then
                  mv -- "${'$'}ready_backup" "${'$'}ready" || true
                fi
                echo 'SIFTALPHA_ERROR=ENVIRONMENT_ACTIVATION_FAILED'
                exit 76
              fi
            ...
            rollback_prepare() {
              ...
              if [ "${'$'}had_previous_env" -eq 1 ] && [ -e "${'$'}backup" ]; then
                mv -- "${'$'}backup" "${'$'}venv" || rollback_ok=0
              fi
              if [ "${'$'}had_previous_ready" -eq 1 ] && [ -e "${'$'}ready_backup" ]; then
                if [ "${'$'}rollback_ok" -eq 1 ]; then
                  mv -- "${'$'}ready_backup" "${'$'}ready" || rollback_ok=0
                fi
              fi
...
SIFTALPHA_PYPROJECT_WEB_EXTRA
            ...
            if [ "${'$'}prepared_python_executable" != "${'$'}venv/bin/python" ]; then
              echo 'SIFTALPHA_ERROR=PYTHON_ENVIRONMENT_PREFIX_MISMATCH'
              printf 'SIFTALPHA_PYTHON_EXECUTABLE=%s\n' "${'$'}prepared_python_executable"
    ...
    private fun buildStart(project: RuntimeProjectSpec): String {
        ...
        val launchEntrypoint = invocation?.entrypoint.orEmpty()
        ...
        val runnerContent = """
            ...
            launch_entrypoint=${sh(launchEntrypoint)}
            launch_args=()
