...
object RuntimeEnvironmentComposer {
    ...
    fun prepare(
        ...
        shellScript = """
            ...
            # Supplemental frontend assets can be an input to Python packaging itself. Build the
            # detected Node/Vite component first so pyproject/setup backends can consume generated
            # dist files during editable install instead of failing on a missing force-include.
            node_output="${'
...
}(bash -lc ${host.sh(node.shellScript)} 2>&1)"
            node_code=${'
        label = "$projectName · 准备环境",
        description = "$projectName · 准备 Python 与检测到的 Node.js Web 组件",
    )
...
}node_output" | grep -Eq '^SIFTALPHA_NODE_ENV=(READY|NOT_REQUIRED)${'
        label = "$projectName · 准备环境",
        description = "$projectName · 准备 Python 与检测到的 Node.js Web 组件",
    )
...
}(bash -lc ${host.sh(primary.shellScript)} 2>&1)"
            primary_code=${'
        label = "$projectName · 准备环境",
        description = "$projectName · 准备 Python 与检测到的 Node.js Web 组件",
    )
