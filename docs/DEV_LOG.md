...
## r47 SiftAlphaX Storage Ownership Consolidation

- Goal: unify all SiftAlpha-owned app-private runtime data beneath a single fixed `SiftAlphaX/` root while keeping user-selected source trees outside app ownership.
- New registry: `SiftAlphaStorage` is the single path authority for app-owned storage.
- Fixed root: `filesDir/SiftAlphaX/`.
- Layout version marker: `SiftAlphaX/layout.info` with `SIFTALPHA_STORAGE_LAYOUT=1`, `ROOT=SiftAlphaX`, `OWNERSHIP=APP_PRIVATE`, `USER_SOURCE_EXCLUDED=1`.
- Owned subtrees now include runtimes, environments, projects, sessions, logs, results, wheelhouse, state and temp.
- Embedded CPython now uses `SiftAlphaX/runtimes/cpython`, `SiftAlphaX/environments/cpython`, `SiftAlphaX/sessions/cpython`, `SiftAlphaX/projects` and `SiftAlphaX/wheelhouse`.
- Internal Alpine now uses `SiftAlphaX/runtimes/alpine`, `SiftAlphaX/environments/alpine`, `SiftAlphaX/sessions/alpine`, `SiftAlphaX/state/pids/alpine` and `SiftAlphaX/temp/proot`.
- Result Web output now uses `SiftAlphaX/results`.
- R47 deliberately performs one development-stage reset of known legacy app-private roots (`filesDir/siftalphax`, `filesDir/siftalpha-results`, legacy Alpine temp/PID cache roots). It does not migrate those old environments.
- The reset happens only when the layout marker is missing or invalid. Once layout 1 is committed, later launches preserve the new tree.
- Runtime lifecycle / operation / Web state / result index preferences are cleared during that one-time reset so the UI cannot falsely claim deleted legacy environments are READY.
- User-selected SAF source locations, project source code, runtime selection and project secrets are not deleted by the reset.
- Android-managed SharedPreferences / Keystore and the no-backup Runtime Agent token remain in platform-standard locations; uninstall still removes them with the app.
- Version: `0.8.0-alpha43-r47`, versionCode `177`.
- Core code head: `be971cce23240fee51cdee8546eecaa0ed32b582`.
- W0 Cloud Build #304: SUCCESS.
- Internal Alpine Probe #58: SUCCESS.
