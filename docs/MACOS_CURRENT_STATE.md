...
状态：**M1 — Core Boundary（核心边界）进行中；M1.1、M1.2、M1.3 已通过，M1.4 — Platform Storage Interface（平台存储接口）= Cloud PASS（云端通过），Real Device Acceptance（真机验收）待完成。**

当前工作分支：
- `feature/cross-platform-core`

当前跨平台 Core（核心）版本：
- `0.8.0-alpha43-r48d11-m1.4`
- Android（安卓） versionCode = `229`
