...
| ID | 操作 | 期望 | 状态 |
|---|---|---|---|
| L-01 | 旧数据以字符串 `"true"/"false"` 保存 | 正确读取并迁移为 Boolean，不闪退 | 待云端/待真机 |
| L-02 | 新数据以 Boolean 保存 | 正确读取环境状态、Runtime 状态和失败原因 | 待云端/待真机 |
| L-03 | 迁移后重新创建 Store 再读取 | 结果保持一致，SharedPreferences 类型为 Boolean | 待云端/待真机 |
| L-04 | 空数据或异常类型 | 回退 UNKNOWN/null，不导致运行中心闪退 | 待云端/待真机 |
