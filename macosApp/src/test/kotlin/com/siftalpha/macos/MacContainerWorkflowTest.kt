...
class MacContainerWorkflowTest {
    ...
    @Test
    fun managedDnsVmResolverHealthRejectsMissingAndLoopbackResolvers() {
        assertTrue(
            MacManagedContainerDns.hasUsableVmResolver(
                """
                # Generated by Colima
        ...
        assertTrue(!MacManagedContainerDns.hasUsableVmResolver(""))
    }

    @Test
    fun managedDnsRendersRecoveryResolvConfFromValidatedResolvers() {
        val rendered = MacManagedContainerDns.renderVmResolvConf(
            listOf("1.1.1.1", "8.8.8.8", "::1", "1.1.1.1"),
        )

        assertTrue(rendered.contains("nameserver 1.1.1.1"))
        assertTrue(rendered.contains("nameserver 8.8.8.8"))
        assertTrue(!rendered.contains("nameserver ::1"))
        assertEquals(1, Regex("nameserver 1\\.1\\.1\\.1").findAll(rendered).count())
        assertTrue(MacManagedContainerDns.hasUsableVmResolver(rendered))
    }
